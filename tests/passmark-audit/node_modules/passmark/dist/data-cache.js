 ...
 */
function getRedisKey(executionId) {
...
async function getGlobalValues(executionId) {
    ...
    const values = await redis_1.redis.hgetall(key);
...
async function saveGlobalValues(executionId, values) {
    ...
    await redis_1.redis.hset(key, values);
    // Set TTL
    await redis_1.redis.expire(key, constants_1.GLOBAL_VALUES_TTL_SECONDS);
    logger_1.logger.debug(`Saved global values to Redis for execution: ${executionId}`);
}
// =============================================================================
 ...
 */
function getProjectDataRedisKey(projectId) {
