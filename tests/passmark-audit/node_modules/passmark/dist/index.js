...
const runSteps = async ({ page, test, expect, userFlow, steps, auth, bypassCache = false, onStepStart, onStepEnd, onReasoning, assertions, projectId, executionId, failAssertionsSilently, }) => {
    ...
    for (let i = 0; i < processedSteps.length; i++) {
        ...
        const cachedStep = redis_1.redis ? await redis_1.redis.hgetall(`step:${userFlow}:${step.description}`) : {};
        if (!bypassCache &&
            !isPlaywrightRetry &&
            !step.bypassCache &&
            cachedStep &&
        ...
        try {
            ...
            if (allToolCalls.length === 1 && redis_1.redis) {
                const cacheData = getPendingCacheData();
                if (cacheData) {
                    await redis_1.redis.hset(`step:${userFlow}:${step.description}`, cacheData);
